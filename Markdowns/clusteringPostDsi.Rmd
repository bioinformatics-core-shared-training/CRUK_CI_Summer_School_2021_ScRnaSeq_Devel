...
sce0 <- readRDS("../CourseMaterials/Robjects/caron_postDeconv_5hCellPerSpl.Rds")
...
sce <- readRDS("../CourseMaterials/Robjects/DataIntergration_mnn.out.rds")
...
```{r show_leiden_graph, warning = FALSE, fig.height=6, out.width = '100%'}
## Define a set of colors to use (must be at least as many as the number of
## communities)
##cols <- RColorBrewer::brewer.pal(n = nlevels(sce$cluster), name = "RdYlBu")
##cols <- grDevices:::colorRampPalette(RColorBrewer::brewer.pal(nlevels(sce$cluster), "RdYlBu"))
...
```{r show_leiden_tsne_2, fig.width=2*5, fig.height=ceiling(nlevels(droplevels(sce$SampleGroup))/2)*5}
# facet by sample group:
p + facet_wrap(~ sce$SampleGroup)
```
