...
```{r setup_pseudotime1, include=FALSE, echo=FALSE, dev="CairoPNG"}
...
```{r uniquifyFeatureNames_pseudotime1}
...
```{r cellType_heatmap_pseudotime1}
...
```{r PRET1_downSample_pseudotime2}
...
```{r PRET1_cellNumber_pseudotime2}
table(colData(x)$Barcode %in% vec.bc)
```

Subset cells from the main SCE object:

```{r PRET1_sce_pseudotime2}
...
```{r PRET1_tempExprGenes_comp_pseudotime2}
...
```{r PRET1_tempExprGenes_top_pseudotime2}
# Identify genes with the most significant time-dependent model fit.
topgenes <- names(sort(gam.pval, decreasing = FALSE))[1:30]  
```
...
```{r PRET1_tempExprGenes_heatmap_pseudotime2}
heatmapdata <- Y[topgenes,]
heatmapdata <- heatmapdata[,order(t, na.last = NA)]
t_ann<-as.data.frame(t)
colnames(t_ann)<-"pseudotime"
...
```{r PRET1_tempExprGenes_enricherH_pseudotime2}
...
```{r PRET1_tempExprGenes_enricherC6_pseudotime2}
...
```{r align_caronPRET1_pseudotime3}
