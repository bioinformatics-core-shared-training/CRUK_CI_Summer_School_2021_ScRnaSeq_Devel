...
<!-- sampleSheet noy used -->

```{r, sampleSheet, message=FALSE, eval=FALSE}
...
<!-- plot variance decomposition data --> 

```{r, eval=FALSE}
combDf <- data.frame(rowData(uncorrected))
table(combDf$chosenHvg, -log10(combDf$FDR) < 0.05)

# plot
xlimVec <- quantile(combDf$tech, probs=c(0.05, 0.95))
ylimVec <- quantile(combDf$bio, probs=c(0.05, 0.95))
p <- ggplot(combDf,
            aes(x=tech, y=bio, col=-log10(FDR))
            ) +
  geom_point(size = 0.5, alpha = 0.5) +
  scale_x_continuous(limits = xlimVec) +
  scale_y_continuous(limits = ylimVec)
p
p + facet_wrap(chosenHvg~FDR<0.05)
```
...
saveRDS(mnn.out, file="DataIntegration_mnn.out.rds")
