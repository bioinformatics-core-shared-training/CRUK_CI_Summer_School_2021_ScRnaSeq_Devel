...
<!-- calc_cell_RLE function - positive median -->

```{r, eval=FALSE}
# calculate RLE for gene whose median is positive.
# ok for non-sparse matrix eg smart-seq
# but not sparse 10X
...
prepRleMat <- function(rleMat, sce = sce) {
	# count NAs for each cell
	nbNonNaPerCell <- colSums(!is.na(rleMat))
	...
	colDataTmp$nbNonNaPerCell <- nbNonNaPerCell
...
myRlePlot <- function(ddToShow, bcToGet=NULL, sortBool=TRUE, qCutBool = TRUE) {
  ...
  # filter gene if necessary
	nbNonNaPerGene <- rowSums(!is.na(deconvRleMat))
	tmpDf <- ddToShow %>%
	  group_by(geneId) %>%
	  summarise(nbNonNaPerGene=sum(!is.na(rle))) %>%
	  #deframe()
	  dplyr::select(geneId, nbNonNaPerGene)

	ddToShow <- ddToShow %>%
		  left_join(tmpDf, by="geneId")

	  if(qCutBool) {
	qCut <- quantile(tmpDf$nbNonNaPerGene, prob=0.95)
	  ddToShow <- ddToShow %>% dplyr::filter(nbNonNaPerGene > qCut) 
	}
  # compute median RLE for each cell across genes
	...
	p <- ggplot(ddToShow, aes(x=Barcode, y=rle,
				  #shape=SampleName,
				  col=nbNonNaPerCell)) +
...
#      )
      ...
      posCountInd <- which(x > 0)
      y <- x[posCountInd]
      z[posCountInd] <- lapply(y,
			       function(i) { log((i + 1)/(medy + 1))/log(2) }
			       )
      z <- unlist(z)
      z
    }
	...
	z[posCountInd] <- y - median(y)
	#print(x[posCountInd][1:3])
      z
    }

        RLE_gene_sct <- function(x) {
      x <- unlist(x)
	z <- x - median(x)
      z
    }
	...
	RLE_matrix <- t(apply(expr_mat, 1, RLE_gene_sct))
    } else {
...
```{r}
# prepare long RLE matrix for plotting
...
```{r}
# plot RLE
# no sorting of cells on RLE
p2.raw <- myRlePlot(dd2, sortBool=FALSE)
p2.raw
```

```{r}
# plot RLE
# with sorting of cells on RLE for each sample
p2.raw.b <- myRlePlot(dd2, sortBool=TRUE)
p2.raw.b
```
...
<!-- rawRleMat - positive median -->

```{r, eval=FALSE}
rawRleMat <- calc_cell_RLE(counts(sce)) # for genes with positive median
...
```{r, eval=FALSE}
# use base boxplot()
# ok but already have ggplot version
...
# ok but positive median, and base boxplot and slow
...
<!-- RLE - positive median - in-house plotRLE -->
...
```{r plot_rle_logRaw_Caron_5hCellPerSpl_ex, cache.lazy = FALSE, eval=FALSE}
...
# sort cell by increasing RLE, for each sample
...
```{r, eval=FALSE}
# 2.2% of points are not NA
# 390 genes out of 17700
...
```{r, eval=FALSE}
# fine
# but with positive-median genes
# only 390 out of 17700
