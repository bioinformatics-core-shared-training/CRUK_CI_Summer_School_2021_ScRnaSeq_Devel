...
# Data integration - PBMMC and PRE-T {#dsi`r anaStg`-PBMMC-PRE-TTop}
...
```{r variables_dsi{{setSuf}}_PBMMC_PRE-T}
...
```{r setup_dsi{{setSuf}}_PBMMC_PRE-T, include=FALSE, echo=FALSE, dev="CairoPNG"}
...
```{r library_dsi{{setSuf}}_PBMMC_PRE-T, include=FALSE}
library(ggplot2)
library(scater)
library(scran)
library(batchelor)
...
```{r load_dsi{{setSuf}}_PBMMC_PRE-T}
...
```{r readSplSht_dsi{{setSuf}}_PBMMC_PRE-T}
# CaronBourque2020
...
```{r renameSamples_dsi{{setSuf}}_PBMMC_PRE-T}
...
```{r subset_dsi{{setSuf}}_PBMMC_PRE-T}
...
```{r stdWkf_dsi{{setSuf}}_PBMMC_PRE-T}
#--- normalization ---#
# use logNormCounts()
...
```{r prep_dsi{{setSuf}}_PBMMC_PRE-T}
...
```{r universe_dsi{{setSuf}}_PBMMC_PRE-T}
...
```{r multiBatchNorm_dsi{{setSuf}}_PBMMC_PRE-T}
...
```{r combineVar_dsi{{setSuf}}_PBMMC_PRE-T}
...
```{r diagPrep_dsi{{setSuf}}_PBMMC_PRE-T}
...
```{r diagClu_dsi{{setSuf}}_PBMMC_PRE-T}
# build shared nearest-neighbour graph
snn.gr <- buildSNNGraph(uncorrected, use.dimred="PCA", BPPARAM=bpp)
...
```{r diagPlotDraw_dsi{{setSuf}}_PBMMC_PRE-T, fig.height=2/3*figSize, fig.width=figSize}
...
```{r diagTsneDraw_dsi{{setSuf}}_PBMMC_PRE-T}
set.seed(1111001)
...
```{r diagTsnePlot1_dsi{{setSuf}}_PBMMC_PRE-T}
p
```

```{r diagTsnePlotSplit_dsi{{setSuf}}_PBMMC_PRE-T, fig.height=figSize*1/2, fig.width=figSize}
...
```{r diagWriteRds_dsi{{setSuf}}_PBMMC_PRE-T}
splSetToGet2 <- gsub(",", "_", splSetToGet)
# save object
...
```{r linReg_rescaleBatches_dsi{{setSuf}}_PBMMC_PRE-T}
...
```{r linReg_diagClu_dsi{{setSuf}}_PBMMC_PRE-T}
...
snn.gr <- buildSNNGraph(rescaled2, use.dimred="PCA", BPPARAM=bpp)
...
```{r linReg_diagPlotDraw_dsi{{setSuf}}_PBMMC_PRE-T, fig.height=2/3*figSize, fig.width=figSize}
tmpMatTab <- table(tmpMat)
sortVecNames <- tmpMatTab %>% rowSums %>% sort(decreasing=TRUE) %>% names
tmpMat$clusters <- factor(tmpMat$clusters, levels=sortVecNames)
tmpMatDf <- tmpMatTab[sortVecNames,] %>% data.frame()
...
```{r linReg_diagTsneDraw_dsi{{setSuf}}_PBMMC_PRE-T}
rescaled2 <- runTSNE(rescaled2, dimred="PCA", BPPARAM=bpp, n_threads=bpnworkers(bpp))
rescaled2$batch <- factor(rescaled2$batch)
p <- plotTSNE(rescaled2, colour_by="batch")
```

```{r linReg_diagTsnePlot1_dsi{{setSuf}}_PBMMC_PRE-T, fig.height=figSize, fig.width=figSize}
p
```

```{r linReg_diagTsnePlotSplit_dsi{{setSuf}}_PBMMC_PRE-T, fig.height=figSize*1/2, fig.width=figSize}
p + facet_wrap(~uncorrected$source_name)
```
...
```{r fastmnnRun_dsi{{setSuf}}_PBMMC_PRE-T}
...
```{r fastmnnGlance_dsi{{setSuf}}_PBMMC_PRE-T}
...
```{r fastmnnRbindColData_dsi{{setSuf}}_PBMMC_PRE-T}
...
```{r fastmnn_clu_dsi{{setSuf}}_PBMMC_PRE-T}
...
```{r fastmnn_diagPlotDrawSampleName_dsi{{setSuf}}_PBMMC_PRE-T, fig.height=2/3*figSize, fig.width=figSize}
...
```{r fastmnn_diagPlotDrawSampleType_dsi{{setSuf}}_PBMMC_PRE-T, fig.height=2/3*figSize, fig.width=figSize}
mnn.out$source_name <- uncorrected$source_name # cell order is maintained by scran functions
...
```{r fastmnn_diagVarComp_dsi{{setSuf}}_PBMMC_PRE-T}
...
```{r fastmnn_diagTsnePlot1_dsi{{setSuf}}_PBMMC_PRE-T}
set.seed(0010101010)
mnn.out <- runTSNE(mnn.out, dimred="corrected", BPPARAM=bpp, n_threads=bpnworkers(bpp))
...
```{r fastmnn_diagTsnePlot2_dsi{{setSuf}}_PBMMC_PRE-T, fig.height=figSize*1/2, fig.width=figSize}
p + facet_wrap(. ~ mnn.out$source_name)
```

Write mnn.out object to file. <!-- for diff exp/abun btw cond -->

```{r fastMnn_writeRds_dsi{{setSuf}}_PBMMC_PRE-T}
...
```{r fastmnn_diagLostVar_dsi{{setSuf}}_PBMMC_PRE-T}
...
#```{r fastmnn_diagTsneB_dsi{{setSuf}}_PBMMC_PRE-T}
genex <- ensToShow[1]
...
#```{r fastmnn_diagTsneT_dsi{{setSuf}}_PBMMC_PRE-T}
genex <- ensToShow[3]
p <- plotTSNE(mnn.out, colour_by = genex, by_exprs_values="reconstructed")
p <- p + ggtitle(
		paste("T cells", genex,
...
#```{r fastmnn_diagTsneM_dsi{{setSuf}}_PBMMC_PRE-T}
genex <- ensToShow[2]
p <- plotTSNE(mnn.out, colour_by = genex, by_exprs_values="reconstructed")
p <- p + ggtitle(
...
#```{r fastmnn_diagTsneE_dsi{{setSuf}}_PBMMC_PRE-T}
genex <- ensToShow[4]
p <- plotTSNE(mnn.out, colour_by = genex, by_exprs_values="reconstructed")
p <- p + ggtitle(
...
```{r fastmnn_diagTsneCellTypeMarkerShow_dsi{{setSuf}}_PBMMC_PRE-T}
...
```{r uncorr_diagTsneB_dsi{{setSuf}}_PBMMC_PRE-T}
# B cells
genex <- ensToShow[1]
...
#```{r uncorr_diagTsneT_dsi{{setSuf}}_PBMMC_PRE-T}
genex <- ensToShow[3]
p <- plotTSNE(uncorrected, colour_by = genex)
p <- p + ggtitle(
		paste("T cells", genex,
...
#```{r uncorr_diagTsneM_dsi{{setSuf}}_PBMMC_PRE-T}
genex <- ensToShow[2]
p <- plotTSNE(uncorrected, colour_by = genex)
p <- p + ggtitle(
		paste("monocytes", genex,
...
#```{r uncorr_diagTsneE_dsi{{setSuf}}_PBMMC_PRE-T}
genex <- ensToShow[4]
p <- plotTSNE(uncorrected, colour_by = genex)
p <- p + ggtitle(
		paste("erythrocytes", genex,
...
```{r uncorr_diagTsneCellTypeMarkerShow_dsi{{setSuf}}_PBMMC_PRE-T}
...
```{r exerCellTypeMarkerList_dsi{{setSuf}}_PBMMC_PRE-T, eval=FALSE}
genesToShow2 <- c(
...
```{r exerCellTypeMarkerList2_dsi{{setSuf}}_PBMMC_PRE-T, eval=FALSE}
...
```{r exerCellTypeMarkerPlot_dsi{{setSuf}}_PBMMC_PRE-T, eval=FALSE}
...
```{r biolHet_comp_dsi{{setSuf}}_PBMMC_PRE-T, warning=FALSE}
...
```{r biolHet_plotDraw_dsi{{setSuf}}_PBMMC_PRE-T, warning=FALSE}
...
```{r biolHet_plotShowHeatmap_dsi{{setSuf}}_PBMMC_PRE-T}
...
```{r biolHet_plotShowTree_dsi{{setSuf}}_PBMMC_PRE-T, fig.height=figSize*length(treeList)*2/3, fig.width=figSize, eval=FALSE}
...
```{r biolHet_plotDrawTree2_dsi{{setSuf}}_PBMMC_PRE-T, eval=FALSE}
...
```{r biolHet_plotShowTree2_dsi{{setSuf}}_PBMMC_PRE-T, fig.height=figSize, fig.width=figSize, eval=FALSE}
gridExtra::grid.arrange(grobs = treeList,
      ncol=2
)
```
...
```{r coasProb_plot_dsi{{setSuf}}_PBMMC_PRE-T, warning=FALSE}
...
```{r randIndex_comp_dsi{{setSuf}}_PBMMC_PRE-T, warning=FALSE}
...
```{r randIndex_plot_dsi{{setSuf}}_PBMMC_PRE-T}
...
```{r consis_comp_dsi{{setSuf}}_PBMMC_PRE-T}
...
```{r consis_fastmnn_dsi{{setSuf}}_PBMMC_PRE-T}
set.seed(1000110)
...
```{r consis_plotTsne_dsi{{setSuf}}_PBMMC_PRE-T}
plotTSNE(mnn.out2, colour_by="batch", shape_by="source_name")
```

```{r consis_plotTsneBySampleType_dsi{{setSuf}}_PBMMC_PRE-T, fig.height=figSize*1/2, fig.width=figSize}
plotTSNE(mnn.out2, colour_by="source_name") +
	facet_wrap(~colData(mnn.out2)$source_name, ncol=2)
```

```{r consis_plotTsneByBatch_dsi{{setSuf}}_PBMMC_PRE-T, fig.height=figSize, fig.width=figSize}
plotTSNE(mnn.out2, colour_by="batch", shape_by="source_name") +
...
```{r consis_plotTsneCluBySample_dsi{{setSuf}}_PBMMC_PRE-T}
plotList <- vector(mode = "list", length = length(splVec))
...
```{r consis_uncorr_findMarkers_dsi{{setSuf}}_PBMMC_PRE-T}
...
```{r consis_uncorr_topGene_dsi{{setSuf}}_PBMMC_PRE-T}
...
```{r consis_uncorr_tsneWoEncour_dsi{{setSuf}}_PBMMC_PRE-T, fig.height=figSize*1/2, fig.width=figSize}
genex <- rownames(demo)[1]
genex <- demo %>% data.frame %>%
  filter(!str_detect(Symbol, "^RP")) %>%
  filter(ensembl_gene_id %in% rownames(mnn.out)) %>%
...
```{r consis_uncorr_tsneWiEncour_dsi{{setSuf}}_PBMMC_PRE-T, fig.height=figSize*1/2, fig.width=figSize}
#genex <- rownames(demo)[1]
...
```{r consis_mnnoutSaveRds_dsi{{setSuf}}_PBMMC_PRE-T}
...
```{r idPbmmcClu_barPlotDescSize_dsi{{setSuf}}_PBMMC_PRE-T, fig.height=figSize*1/2, fig.width=figSize}
mnn.out$source_name <- uncorrected$source_name # cell order is maintained by scran functions
tmpMat <- data.frame("clusters"=clusters.mnn, "batch"=mnn.out$source_name)
tmpMatTab <- table(tmpMat)
sortVecNames <- tmpMatTab %>% rowSums %>% sort(decreasing=TRUE) %>% names
...
```{r idPbmmcClu_barPlotDescPropPbmmc_dsi{{setSuf}}_PBMMC_PRE-T}
...
```{r idPbmmcClu_contingTabCluByBatch_dsi{{setSuf}}_PBMMC_PRE-T}
...
```{r idPbmmcClu_barPlotDescPropPbmmcSampleName_dsi{{setSuf}}_PBMMC_PRE-T, eval=FALSE}
...
```{r idPbmmcClu_contingTabCluByType_dsi{{setSuf}}_PBMMC_PRE-T}
tab.mnn <- table(Cluster=clusters.mnn,
		 Batch=as.character(mnn.out$source_name))
##tab.mnn
...
```{r idPbmmcClu_barPlotDescPropPbmmcSampleType_dsi{{setSuf}}_PBMMC_PRE-T, eval=FALSE}
norm2 <- normNoLog %>%
  data.frame() %>%
  select(-sum, -prop) %>%
  tibble::rownames_to_column("clusters") %>% 
...
```{r idPbmmcClu_barPlotPropPbmmcThd050_dsi{{setSuf}}_PBMMC_PRE-T, fig.height=figSize*1/2, fig.width=figSize}
...
```{r idPbmmcClu_barPlotPropPbmmcTsneThd050_dsi{{setSuf}}_PBMMC_PRE-T, fig.height=figSize, fig.width=figSize}
...
```{r idPbmmcClu_barPlotPropPbmmcThd080_dsi{{setSuf}}_PBMMC_PRE-T, fig.height=figSize*1/2, fig.width=figSize}
...
```{r idPbmmcClu_barPlotPropPbmmcTsneThd080_dsi{{setSuf}}_PBMMC_PRE-T, fig.height=figSize, fig.width=figSize}
# cluster:
p <- plotTSNE(mnn.out, colour_by = "cluster", by_exprs_values="reconstructed")
p <- p + ggtitle("clusters")
...
```{r idPbmmcClu_barPlotPropPbmmcTsneThd050CellTypeMarkers_dsi{{setSuf}}_PBMMC_PRE-T, fig.height=figSize*1/2, fig.width=figSize, eval=!FALSE}
for (genex in ensToShow)
{
	p <- plotTSNE(mnn.out, colour_by = genex, by_exprs_values="reconstructed") +
...
```{r idPbmmcClu_select_dsi{{setSuf}}_PBMMC_PRE-T, fig.height=figSize*1/2, fig.width=figSize}
...
```{r idPbmmcClu_barPlotPropPbmmcTsneSelec_dsi{{setSuf}}_PBMMC_PRE-T, fig.height=0.5*figSize, fig.width=figSize, eval=!FALSE}
# cluster:
p <- plotTSNE(mnn.out, colour_by = "cluster", by_exprs_values="reconstructed")
p <- p + ggtitle("clusters")
...
```{r idPbmmcClu_normNoLogSaveRds_dsi{{setSuf}}_PBMMC_PRE-T}
splSetToGet2 <- gsub(",", "_", splSetToGet)
# save object
...
<details>
```{r sessionInfo_dsi{{setSuf}}_PBMMC_PRE-T}
sessionInfo()
```
</details>
