...
subtitle: 'Quality Control - Challenge'
...
batches for the adaptive filters, and at what level the "batch" should be 
defined.
...
We will load the Caron data sets. We have already prepared a sample
...
object are the ones you are expecting (all of the Caron data set). Add your 
code into the code chunk below.

```{r}

```

For the purposes of visualisation we want to be able to distinguish between the
two libraries for PBMMC_1. For this reason we have added suffixes "a" and "b".
Later in we will remove these suffixes. In the downstream analyses will want the
cells to be treated as having come from a single samples.
...
Let's have a look at how these filters would affect the data set. Replace
`XXXXXXX` with the appropriate column name from the `colData`.
...
batch-wise analysis and the analysis using all samples. You'll need
to replace `XXXXXXX` with the appropriate vector for the original filtering, and
the appropriate slot of the `batch.reasons` object.

```{r}
all.thresholds <- tibble(`Batch`="All",
       `Library Size`=attr(XXXXXXX, "thresholds")[1],
       `Genes detected`=attr(XXXXXXX, "thresholds")[1],
       `Mitochondrial UMIs`=attr(XXXXXXX, "thresholds")[2])


tibble(`Batch`=names(attr(batch.reasons[[1]], "thresholds")[1,]),
       `Library Size`=attr(batch.reasons$XXXXXXX, "thresholds")[1,],
       `Genes detected`=attr(batch.reasons$XXXXXXX, "thresholds")[1,],
       `Mitochondrial UMIs`=attr(batch.reasons$XXXXXXX, "thresholds")[2,]) %>% 
...
Replace the filter columns in the droplet annotation with your chosen filters
and then create a fourth column called `discard` that combines the three filters
into a single boolean vector.

```{r set_filter_columns}

```

Now exclude poor-quality cells from the SingleCellExperiment object and save the
