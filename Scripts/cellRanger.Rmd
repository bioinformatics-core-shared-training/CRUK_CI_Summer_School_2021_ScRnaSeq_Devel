...
it is a relatively straightforward process and you can find comprehensive
instructions
...
placed in a single directory. Cell Ranger will be run separately on each file,
you will need to provide the software with the index (e.g. SIGAA6) of the sample
...
Cell Ranger will create a single folder for the sample names as per the `--id`
option in the command. In the top level of the directory is a directory called
`outs`, this contains the results of the analysis pipeline. There are also many
intermediate and log files/directories that will not be of immediate interest.
...
the count matrix directories in to R.
...
The CellRanger summary report - `web_summary.html` - is a very useful first 
port of call for assessing the quality of the data. An example is linked
[her](Slides/web_summary.html).

The first page contains various QC metrics about sequencing quality, mapping
quality and cell calling. The second page of the report provides some basic
analysis of droplets that Cell Ranger has identified as potentially containing
cells, including some basic clustering and gene expression analysis. This page
is interactive, allowing you to some very basic odata exploration.
...
that contain good quality cells and empty droplets. The filtered counts matrices
only contain data for droplets that CellRanger has determined to be cells. It 
performs this filtering in two steps.

1. UMI threshold based
  ...
  hard UMI threshold above which all droplets are called as cells.

2. RNA profile comparison with background
  ...
  (this will usually be 100's of thousands). This set of droplets is determined
  to definitely not contain cells and as such any RNA detected is merely
  background within the media (due to damages cells leaking RNA). A RNA profile
  is derived for this background. The RNA profile of all droplets with more than
  100 UMIs is then compared to this background profile. Any droplet with a
  profile that differs significantly from the background profile is called as a
  cell.
