...
```{r, eval=FALSE}
sce
# get raw count RLE matrix
# with one row per gene
# and one column per cell
## rawRleMat0 <- calc_cell_RLE(counts(sce)[1:10,]) # 10 first genes

# calc_cell_RLE only uses genes whose median UMI count is greater than 0.

# so let's see how many such genes there are.

# get for each gene the median across cells
medVec <- apply(counts(sce), 1, median)
# cont genes whose median is greater than 0
table(medVec > 0)
# get indices for these genes
# to focus on them for now
posMedianInd <- which(medVec > 0)
# sub set matrix and keep these genes only
# or even only some of them
#rawRleMat0 <- calc_cell_RLE(counts(sce)[posMedianInd[1:10],])
rawRleMat0 <- calc_cell_RLE(counts(sce)[posMedianInd,])
colnames(rawRleMat0) <- sce$Barcode
...
```{r, eval=FALSE}
# check boxplot: too busy,
# with border
p <- ggplot(dd2, aes(x=Barcode, y=rle)) + 
  geom_boxplot()

# colour by SampleGroup
p <- ggplot(dd2, aes(x=Barcode, y=rle, colour=SampleName)) + 
  geom_boxplot()
 
p + facet_wrap(~SampleGroup, ncol=2)
          
# only keep 10 cells for now.          
bcToGet <- gsub("-",".",colData(sce)$Barcode[1:10])
dd10 <- dd2 %>% filter(Barcode %in% bcToGet)
ggplot(dd10, aes(x=Barcode, y=rle)) + 
  geom_boxplot()
```
...
scater___plotRLE_full
function (df, aesth, ncol, ...) 
{
    plot_out <- ggplot(df, aesth) + geom_boxplot(...) + stat_summary(geom = "crossbar", 
        width = 0.65, fatten = 0, color = "white", fun.data = function(x) {
...
#<bytecode: 0x55f94f03bf60>
#<environment: namespace:scater>
```

```{r, eval=FALSE}
p <- ggplot(dd10, aes(x=Barcode, y=rle)) + 
  geom_boxplot()

p <- ggplot(dd10, aes(x=Barcode, y=rle, col=SampleGroup))
#p + stat_summary(geom = "crossbar",
```

<!-- DEV -->

```{r, eval=FALSE}
#bcToGet <- gsub("-",".",colData(sce)$Barcode[1:1000])
bcToGet <- gsub("-",".",colData(sce)$Barcode)
ddToShow <- dd2 %>% filter(Barcode %in% bcToGet)
...
```{r, eval=FALSE}
# DEV
p <- ggplot(dd, aes(x=Barcode, y=rle)) + 
  geom_boxplot()
#p

p <- ggplot(dd2, aes(x=Barcode, y=rle))
p + stat_summary(geom = "crossbar", 
        width = 0.05, fatten = 0, color = "black", fun.data = function(x) {
            c(y = median(x), ymin = median(x), ymax = median(x))
        }) + ylab("Relative log expression") + xlab("Sample") + 
        theme_classic() + theme(axis.text.x = element_blank(), 
        axis.ticks.x = element_blank(), axis.line.x = element_blank())
