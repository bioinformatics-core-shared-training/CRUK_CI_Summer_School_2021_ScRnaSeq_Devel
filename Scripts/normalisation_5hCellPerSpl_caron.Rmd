...
subtitle: 'Normalisation - Caron data seet - 500 cells per sample'
...
# Normalisation - `r "Caron"` set {#NormalisationCaron5hcpsTop}
...
```{r Caron_downsample_variables, eval=TRUE}
setSuf <- "_5hCellPerSpl"
nbCells <- 500
```

```{r Caron_downsample, eval=TRUE}
...
differentially expressed betwenn cells. In `scater` CPMs are computed with the
following code:
...
most genes are not differentially expressed, and the 40% lof genes left after 
...
## Extended materials - Visualisation
...
```{r show_pca_logRaw_Caron_5hCellPerSpl, eval=FALSE}
...
```{r show_rle_logRaw_Caron_5hCellPerSpl, eval=FALSE}
...
```{r show_pca_logCpm_Caron_5hCellPerSpl, eval=FALSE}
tmpFn <- sprintf("%s/%s/%s_sce_nz_postQc%s_%sPca.png",
		 dirRel, qcPlotDirBit, setName, setSuf, typeNorm)
knitr::include_graphics(tmpFn, auto_pdf = TRUE)
rm(tmpFn)
...
```{r show_rle_logCpm_Caron_5hCellPerSpl, eval=FALSE}
tmpFn <- sprintf("%s/%s/%s_sce_nz_postQc%s_%sRle.png",
		 dirRel, qcPlotDirBit, setName, setSuf, typeNorm)
knitr::include_graphics(tmpFn, auto_pdf = TRUE)
rm(tmpFn)
...
```{r show_pca_deconv_Caron_5hCellPerSpl, eval=FALSE}
tmpFn <- sprintf("%s/%s/%s_sce_nz_postQc%s_%sPca.png",
		 dirRel, qcPlotDirBit, setName, setSuf, typeNorm)
knitr::include_graphics(tmpFn, auto_pdf = TRUE)
rm(tmpFn)
...
```{r show_tsne_deconv_Caron_5hCellPerSpl, eval=FALSE}
...
```{r show_umap_deconv_Caron_5hCellPerSpl, eval=FALSE}
...
```{r show_rle_deconv_Caron_5hCellPerSpl, eval=FALSE}
tmpFn <- sprintf("%s/%s/%s_sce_nz_postQc%s_%sRle.png",
		 dirRel, qcPlotDirBit, setName, setSuf, typeNorm)
knitr::include_graphics(tmpFn, auto_pdf = TRUE)
rm(tmpFn)
...
```{r show_pca_sct_Caron_5hCellPerSpl, eval=FALSE}
tmpFn <- sprintf("%s/%s/%s_sce_nz_postQc%s_%sPca.png",
		 dirRel, qcPlotDirBit, setName, setSuf, typeNorm)
knitr::include_graphics(tmpFn, auto_pdf = TRUE)
rm(tmpFn)
...
```{r show_tsne_sct_Caron_5hCellPerSpl, eval=FALSE}
tmpFn <- sprintf("%s/%s/%s_sce_nz_postQc%s_%sTsne.png",
		 dirRel, qcPlotDirBit, setName, setSuf, typeNorm)
knitr::include_graphics(tmpFn, auto_pdf = TRUE)
rm(tmpFn)
...
```{r show_umap_sct_Caron_5hCellPerSpl, eval=FALSE}
tmpFn <- sprintf("%s/%s/%s_sce_nz_postQc%s_%sUmap.png",
		 dirRel, qcPlotDirBit, setName, setSuf, typeNorm)
knitr::include_graphics(tmpFn, auto_pdf = TRUE)
rm(tmpFn)
...
```{r show_rle_sct_Caron_5hCellPerSpl, eval=FALSE}
tmpFn <- sprintf("%s/%s/%s_sce_nz_postQc%s_%sRle.png",
		 dirRel, qcPlotDirBit, setName, setSuf, typeNorm)
knitr::include_graphics(tmpFn, auto_pdf = TRUE)
rm(tmpFn)
