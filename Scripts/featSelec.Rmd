...
```{r libraries_featSelec, message=FALSE, warning=FALSE}
library(ggplot2)
library(scater)
library(scran)
library(Cairo)
...
```{r featSelec_setup, include=FALSE, echo=FALSE, dev="CairoPNG"}
...
```{r featSelec_readIn, cache=FALSE}
# Read object in:
...
```{r plot_count_violin_HVGbot20, eval = TRUE}
