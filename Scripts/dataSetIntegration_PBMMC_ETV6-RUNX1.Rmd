...
```{r library_dsi{{setSuf}}_PBMMC_ETV6-RUNX1, include=FALSE}
library(ggplot2)
library(scater)
library(scran)
library(batchelor)
...
all.sce <- readRDS("DataIntergration_all_sce.rds")
all.sce
```
...
#tab
clusterTab <- table("clusters"=clusters, "batch"=uncorrected$Sample.Name)
```
...
```{r linReg_diagPlotDraw_PBMMC_ETV6-RUNX1 }
...
lp2 <- ggplot(data=clusterTab.linear, aes(x=clusters, fill=batch)) +
  ...
  scale_y_continuous(labels = scales::percent) +
  theme(legend.text = element_text(size = 7))
gridExtra::grid.arrange(lp1, lp2)
```
...
```{r fastmnn_diagPlotDrawSampleName_PBMMC_ETV6-RUNX1, fig.height=6, fig.width=7}
...
gridExtra::grid.arrange(mp1, mp2)
```

Cluster size and cell contribution by sample type, with clusters sorted by size:

```{r fastmnn_diagPlotDrawSampleType_PBMMC_ETV6-RUNX1, fig.height=6, fig.width=7}
...
gridExtra::grid.arrange(mp1.s, mp2.s)
```
...
```{r fastmnn_diagTsnePlot2_PBMMC_ETV6-RUNX1, fig.height=4, fig.width=7}
...
mp.tsne.cluster
#mp.umap.cluster <- plotUMAP(mnn.out, colour_by ="label", shape_by="source_name")
#mp.umap.cluster
```
...
```{r, heatmaps}
tab34 <- table(paste("after", clusters.mnn[linear_rescaled$batch=="GSM3872434"]),
               paste("before", colLabels(rescaled[[1]])))
heat34 <- pheatmap(log10(tab34 +10), cluster_rows = FALSE, cluster_cols = FALSE,
                   main = "GSM3872434", silent = TRUE)
tab35 <- table(paste("after", clusters.mnn[linear_rescaled$batch=="GSM3872435"]),
               paste("before", colLabels(rescaled[[1]])))
heat35 <- pheatmap(log10(tab34 +10), cluster_rows = FALSE, cluster_cols = FALSE,
                   main = "GSM3872435", silent = TRUE)
tab36 <- table(paste("after", clusters.mnn[linear_rescaled$batch=="GSM3872436"]),
               paste("before", colLabels(rescaled[[1]])))
heat36 <- pheatmap(log10(tab34 +10), cluster_rows = FALSE, cluster_cols = FALSE,
                   main = "GSM3872436", silent = TRUE)
tab37 <- table(paste("after", clusters.mnn[linear_rescaled$batch=="GSM3872437"]),
               paste("before", colLabels(rescaled[[1]])))
heat37 <- pheatmap(log10(tab34 +10), cluster_rows = FALSE, cluster_cols = FALSE,
                   main = "GSM3872437", silent = TRUE)
tab42 <- table(paste("after", clusters.mnn[linear_rescaled$batch=="GSM3872442"]),
               paste("before", colLabels(rescaled[[1]])))
heat42 <- pheatmap(log10(tab34 +10), cluster_rows = FALSE, cluster_cols = FALSE,
                   main = "GSM3872442", silent = TRUE)
tab43 <- table(paste("after", clusters.mnn[linear_rescaled$batch=="GSM3872443"]),
               paste("before", colLabels(rescaled[[1]])))
heat43 <- pheatmap(log10(tab34 +10), cluster_rows = FALSE, cluster_cols = FALSE,
                   main = "GSM3872443", silent = TRUE)
tab44 <- table(paste("after", clusters.mnn[linear_rescaled$batch=="GSM3872444"]),
               paste("before", colLabels(rescaled[[1]])))
heat44 <- pheatmap(log10(tab34 +10), cluster_rows = FALSE, cluster_cols = FALSE,
                   main = "GSM3872444", silent = TRUE)

gridExtra::grid.arrange(heat34[[4]],heat35[[4]],heat36[[4]],heat37[[4]],
                        heat42[[4]],heat43[[4]],heat44[[4]])
```
...
```{r consis_mnnoutSaveRds_dsi{{setSuf}}_PBMMC_ETV6-RUNX1, warning=F}
