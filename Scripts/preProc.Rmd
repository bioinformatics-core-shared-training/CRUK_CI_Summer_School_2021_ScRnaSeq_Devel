...
## QC filtering using PCA

In the previous approach we used the three metrics in isolation to filter
droplets. Another approach is to combine the three metrics in a single filtering
step. One way to do this is to perform a principal components analysis (PCA) and
use this to detect outliers. The `scater` function `runColDataPCA` can be used
to perform the PCA and detect outliers. Here we will use the same three metrics
as before: library size, the number of genes detected and the mitochondrial
content.

```{r}
sce <- runColDataPCA(sce, 
                     variables=list("sum", "detected", "subsets_Mito_percent"),
                     outliers=TRUE)
```
...
exclude many good quality cells.
...
```{r sparsity_compute, eval=saveRds}
# compute - SLOW
...
```{r sparsity_readRds, eval=!saveRds}
# Read object in:
tmpFn <- sprintf("%s/%s/Robjects/sce_nz_sparsityCellGene.Rds",
		 projDir, outDirBit)
