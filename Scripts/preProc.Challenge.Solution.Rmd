...
```{r}
colData(sce)
caronSamples <- filter(samplesheet, DatasetName=="Caron") %>% pull(Sample)
all(caronSamples%in%colData(sce)$Sample)
```
...
Replace the columns in the droplet annotation with your chosen filters and then
create a fourth column that combines the three filters into a single boolean
vector.
...
sce$discard <- sce$low_lib_size | low_n_features | high_Mito_percent
```

Now exclude poor-quality cells from the SingleCellExperiment object and save the
final object for later use.
